...
One of the most powerful features of WebLauncher Framework is the ability to link
...
and utilize associations between models in WebLauncher Framework.
...
The four association types in WebLauncher Framework are: hasOne, hasMany,
...
WebLauncher Framework will automatically create links between associated model
...
.. note::
    ...
    It is not mandatory to follow WebLauncher Framework conventions. You can easily override
...
Since 2.0, WebLauncher Framework has supported having multiple ``counterCache`` in a single model
...
-  **finderQuery**: A complete SQL query WebLauncher Framework can use to fetch
...
All right. At this point, you can already call yourself a WebLauncher Framework
...
-  **unique**: boolean or string ``keepExisting``.
    - If true (default value) WebLauncher Framework will first delete existing relationship
      records in the foreign keys table before inserting new ones.
      Existing associations need to be passed again when updating.
    - When false, WebLauncher Framework will insert the specified new relationship records
...
-  **offset**: The number of associated rows to skip over (given
   the current conditions and order) before fetching and associating.
-  **finderQuery**: A complete SQL query WebLauncher Framework can use to fetch associated
...
Join models are pretty useful things to be able to use, and WebLauncher Framework
...
This association creation and destruction is done using the WebLauncher Framework
...
In WebLauncher Framework, some associations (belongsTo and hasOne) perform
...
retrieved data complies with the WebLauncher Framework data structure.
...
Using joins allows you to have maximum flexibility in how WebLauncher Framework handles associations
