...
date. Defining validation rules enables WebLauncher Framework's automagic showing
of error messages in forms if the data submitted does not follow
the defined rules.

WebLauncher Framework has many validation rules and using them can be quite easy.
...
that WebLauncher Framework will use when these validation rules fail.
...
The numeric check is so that WebLauncher Framework does the right thing when
``$value`` is zero.
...
The Validation class in WebLauncher Framework contains many validation rules that
...
.. tip::
    ...
    https://github.com/WebLauncher Framework/localized
