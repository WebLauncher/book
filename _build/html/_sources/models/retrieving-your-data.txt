...
Basically this is just telling WebLauncher Framework to accept the value ``available`` as the first
...
.. note::
    ...
    MySQL only. WebLauncher Framework does not provide any data abstraction when running
...
in one way or another. In general, WebLauncher Framework prefers using arrays for
...
manipulatable parts. This allows WebLauncher Framework to generate the most
...
also enables WebLauncher Framework to secure your queries against any SQL injection attack.

.. warning::

    WebLauncher Framework only escapes the array values. You should **never** put user data
...
Notice the '!=' that follows the field name. WebLauncher Framework can parse out
...
By default, WebLauncher Framework joins multiple conditions with boolean AND.
...
WebLauncher Framework accepts all valid SQL boolean operations, including AND, OR,
...
WebLauncher Framework can also check for null fields. In this example, the query
...
.. note::
    ...
    WebLauncher Framework will quote the numeric values depending on the field
    type in your DB.
